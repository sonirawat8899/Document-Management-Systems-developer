...
class UserController extends Controller
{
    ...
    public function userManagement(){
      ...
      $users = DB::table('users')->select(
        ...
        ->orderBy('id','DESC')->where('users.user_id',$userId)->get();
      return view('user.user.userManagement',['users'=>$users]);
  }

    //Delete function to delete in user body
  public function delete($id){
      DB::delete('delete from users where id = ?',[$id]);
        return redirect('user/userManagement')->with('success', 'User has been deleted successfully.');
  }
  ...
  public function edit($id) {
      ...
      $company_name = DB::table('companies')->select('id','company_name')->get();
        ...
        return view('user.user.edit')->with(['users'=>$users,'project_manager'=>$project_manager,'company_name'=>$company_name]);
  }
      //Update Code
  public function update(Request $request){
    $request->validate(
      ...
      return redirect('user/userManagement')->with('success', 'User has been updated successfully.');
  }
  ...
  public function adduser(){
      $project_manager = DB::table('usertype')->select('id','name')->whereIn('id', [2,0])->get();
      $company_name = DB::table('companies')->select('id','company_name')->get();
      return view('user.user.adduser',['project_manager'=>$project_manager,'company_name'=>$company_name]);
  }
      //Save user data from the user panel
  public function register(Request $request)
  {
      ...
      return redirect('user/userManagement')->with('success', 'User has been added successfully.');
  }
      //change status of user from user panel
